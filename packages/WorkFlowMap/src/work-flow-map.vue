<template>
  ...
  <div @click="removeNode">合并节点1</div>
  <div @click="removeNode">删除节点1</div>
...
const findHeadandLast = ()=>{
  let heads =  nodesList.edges.filter(item=>{
...
onMounted(() => {
  ...
  graph.on('node:click',(item)=>{
    console.log(item.item._cfg.model)
    currentNode.value = item.item._cfg.model
  })
});
</script>
